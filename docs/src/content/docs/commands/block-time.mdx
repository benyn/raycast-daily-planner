...
<YouTube id="eff9h1WYxSo" posterQuality="max" title="Introducing The Time-Block Planner by Cal Newport" />
...
<video
  src="/raycast-daily-planner/assets/block-time.mp4"
  title="Show Menu Bar Timer video walk-through"
  controls
...
- Free from conflicts with events on your ["Time Block Calendar"](/raycast-daily-planner/extension-settings#time-block-calendar) or any of the ["Event Calendars"](/raycast-daily-planner/extension-settings#event-calendars) configured in Raycast Settings.
...
- **Sync First**: Launch the Calendar app, and if you're using Reminders or Things, launch those as well before running the "Block Time for To-Dos" command to sync the latest data from the cloud. The ["Split Screen To-Dos and Calendar"](/raycast-daily-planner/commands/split-screen) action will be useful for this purpose, as it launches your primary to-do list app and Calendar app if they aren't already running.
